...
def create_graph(values_dataframe, axis, graph_id: int, graph_type, x_axis_name, y_axis_name, settings,
    ...
    if isinstance(graph_type, list):
        if not (isinstance(y_axis_name, list) and len(y_axis_name) >= len(graph_type)):
            raise Exception("y_axis_name is not of proper length for multi-graphing.  Make sure y_axis_name is a "
                            "list and of equal length to graph_type")
        if not (isinstance(settings, list) and len(settings) >= len(graph_type)):
            raise Exception("settings is not of proper length for multi-graphing.  Make sure settings is a list and of "
                            "equal length to graph_type.")

        if not x_data:
            x_data = [None for i in range(len(graph_type))]
        if not y_data:
            y_data = [None for i in range(len(graph_type))]

        y_values = []
        for (single_graph_type, single_y_axis_name, single_settings, single_y_data) in zip(graph_type, y_axis_name,
                                                                                           settings, y_data):
        ...
        graph_dictionary = {"figure": axis.get_figure(), "type": graph_type, "id": graph_id}
...
def create_multiple_graphs(urls, graph_configurations):
    ...
    for graph in range(graph_count_number):
        ...
        if "x_axis_label" not in graph_configurations[graph]:
            if graph_configurations[graph]["graph_type"] == "pie":
        ...
        if "y_axis_label" not in graph_configurations[graph]:
            if isinstance(graph_configurations[graph]["graph_type"], list):
        ...
        if "settings" not in graph_configurations[graph]:
            if isinstance(graph_configurations[graph]["graph_type"], list):
                graph_configurations[graph].update(
                    {"settings": [{} for i in range(len(graph_configurations[graph]["graph_type"]))]})
            else:
        ...
        graph_dictionary, returned_graph_configuration = create_graph(
            ...
            axis=axis,
            graph_id=graph + 1,
            graph_type=graph_configurations[graph]["graph_type"],
